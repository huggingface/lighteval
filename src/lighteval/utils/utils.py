...
def make_results_table(result_dict):
    ...
    standard errors, and sample counts. The table includes columns for task name,
    version, number of samples, metric, value, and standard error.
    ...
    Args:
        result_dict (dict): Dictionary containing evaluation results with the structure:
            ...
            - 'config_tasks': (optional) Dict mapping task names to task config objects
              with an ``effective_num_docs`` attribute used to populate the sample count column.
    ...
    Examples:
        ...
        # Returns markdown table with task, version, num. samples, metric, value, ±, stderr columns
    """
    md_writer = MarkdownTableWriter()
    md_writer.headers = ["Task", "Version", "Num. samples", "Metric", "Value", "", "Stderr"]
    ...
    for k in sorted(result_dict["results"].keys()):
        ...
        num_samples = task_config.effective_num_docs if task_config is not None else ""
