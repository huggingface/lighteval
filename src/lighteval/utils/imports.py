...
NO_VLLM_ERROR_MSG = "You are trying to use an VLLM model, for which you need `vllm`, which are not available in your environment. Please install them using pip, `pip install vllm`."
...
NO_SGLANG_ERROR_MSG = "You are trying to use an sglang model, for which you need `sglang` and `flashinfer`, which are not available in your environment. Please install them using pip, `pip install sglang flashinfer`."
