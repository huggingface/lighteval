...
When the evaluated model uses a revision other than the default one (`main`), the revision is
appended to the model name, so that evaluations of several revisions of the same model are not
mixed together: `{output_dir}/results/{model_name}_{revision}/results_{timestamp}.json`.
...
variables: `output_dir`, `model_name`, `org`, `revision`. For example:
`{output_dir}/{org}_{model}/{revision}`. The template will be used to create the path for the results file.
