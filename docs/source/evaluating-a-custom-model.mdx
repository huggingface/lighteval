...
### 1. Error Handling
Implement robust error handling in your model methods to gracefully handle edge cases:
...
def greedy_until(self, docs: List[Doc]) -> List[ModelResponse]:
    try:
        ...
        # Return fallback responses or re-raise as appropriate
        return [ModelResponse(text="", logprob=0.0) for _ in docs]
```

### 2. Batching
Consider implementing efficient batching in your model methods to improve performance:
...
def greedy_until(self, docs: List[Doc]) -> List[ModelResponse]:
    ...
    for i in range(0, len(docs), batch_size):
        ...
        batch_responses = self.generate_batch(batch)
...
### 3. Documentation
Add clear docstrings to your model class and methods explaining any specific requirements or limitations:

```python
class MyCustomModel(LightevalModel):
    """
    Custom model implementation for [your use case].
...
### 4. Caching
Enable caching to speed up repeated evaluations and development iterations:
...
### 5. Testing
Test your custom model implementation thoroughly:

```python
# Test with a small sample
test_docs = [Doc(prompt="Hello", target="World")]
responses = model.greedy_until(test_docs)
